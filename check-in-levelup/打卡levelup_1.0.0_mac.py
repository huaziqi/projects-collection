...
#################################### 初始界面
    ...
    if(done == False):       
        pp_lay = [
        [sg.Text("输入用户名", font = ("隶变-简", 18)), sg.InputText(font = ("兰亭黑-简", 15))],
        [sg.Text("自定义升级经验值", font = ("隶变-简", 18)), sg.InputText(font = ("兰亭黑-简", 15), do_not_clear = False)],
        [sg.OK(size = (100, 0))]
        ]
    ...
    init_lay = [
            [sg.Text('明日复明日，明日何其多。', font = ("行楷-简", 30), size = (45, 1), justification = 'center')],
            [sg.Text("hi, " + name + " !  " + "level: " + str(level), font = ("隶变-简", 23)), sg.Text("Exp:", font = ("隶变-简", 20)), sg.ProgressBar(exp_mod, orientation = 'h', key = "progress_bar", size = (20, 20)), sg.Text(" " + str(now_exp) + "/" + str(exp_mod), font = ("隶变-简", 23))],
            [sg.Text("现有任务：", font = ("隶变-简", 20), size = (20, 1), justification = 'left'), sg.Text("交互窗口",  font = ("隶变-简", 20), size = (30, 0), justification = 'right')],
            ]
    list_value = []
    list_valuere = []
    ...
    init_lay += [sg.Button('添加任务', font = ("隶变-简", 15)), sg.Button('cmd', font = ("隶变-简", 15)), sg.Text("<--更多命令", font = ("隶变-简", 17))],
...
#################################### cmd
        if(init_event == 'cmd'):
            cmd_lay = [[sg.Text('现有指令', font = ("隶变-简", 20))],
                                ...
                                if(cmd_del_isnum == True):
                                    if(cmd_del_num >= 0 and cmd_del_num < len(task)):
                                        if(task[cmd_del_num][3] == 2):
                                            ...
                                            [sg.Text('明日复明日，明日何其多。', font = ("行楷-简", 30), size = (45, 1), justification = 'center')],
                                            [sg.Text("hi, " + name + " !  " + "level: " + str(level), font = ("隶变-简", 23)), sg.Text("Exp:", font = ("隶变-简", 20)), sg.ProgressBar(exp_mod, orientation = 'h', key = "progress_bar", size = (20, 20)), sg.Text(" " + str(now_exp) + "/" + str(exp_mod), font = ("隶变-简", 23))],
                                            [sg.Text("现有任务：", font = ("隶变-简", 20), size = (20, 1), justification = 'left')],
                                ]
                                            list_value = []
                                            list_valuere = []
                                            task_able -= 1
...
#################################### 添加任务
        if(init_event == '添加任务'):
            if(True):
                init_window.close()
                add_lay = [
                    [sg.Text('添加打卡任务', font = ("隶变-简", 25))],
                    [sg.Text('任务:',font = ("隶变-简", 18)), sg.InputText(font = ("兰亭黑-简", 15))],
                    [sg.Text('exp :  ',font = ("隶变-简", 18)), sg.InputText(font = ("兰亭黑-简", 15), do_not_clear=False)],
                    [sg.Text('任务描述', font = ("隶变-简", 18))],
                    [sg.Multiline(font = ("兰亭黑-简", 15),  size = (70, 5))],

                    [sg.Button('添加', font = ("隶变-简", 15)), sg.Button('返回', font = ("隶变-简", 15), key = 'back')]
    ]
                ...
                while True:
                    ...
                    if add_event == '添加' and add_values[0] != '' and add_values[1] != '':
                        ...
                        if(num_flag):
                            ...
                            sg.popup('exp必须为数字！', font = ("隶变-简", 20))
                            

                    elif (add_values[0] == '' or add_values[1] == '') and add_event == '添加':
                        sg.popup('任务或经验不能为空！', font = ("隶变-简", 20))
                    if(add_event == 'back'):
    #################################### 重构init_window
                        init_lay = [
                            [sg.Text('明日复明日，明日何其多。', font = ("行楷-简", 30), size = (50, 1), justification = 'center')],
                            [sg.Text("hi, " + name + " !  " + "level: " + str(level), font = ("隶变-简", 23)), sg.Text("Exp:", font = ("隶变-简", 20)), sg.ProgressBar(exp_mod, orientation = 'h', key = "progress_bar", size = (20, 20)), sg.Text(" " + str(now_exp) + "/" + str(exp_mod), font = ("隶变-简", 23))],
                            [sg.Text("现有任务：", font = ("隶变-简", 20), size = (15, 1), justification = 'left')]
        ]
                        list_value = []
                        list_valuere = []
                        for i in range(0, len(task)):
                        ...
                        [sg.Listbox(values = (list_valuere),enable_events = True, size = (30, min(10, 1 * task_able)), font = ("黑体-简", 20)), sg.Output(size = (30, 10), font = ("兰亭黑-简", 15))],
                                            [sg.Button('添加任务', font = ("隶变-简", 15)), sg.Button('cmd', font = ("隶变-简", 15)), sg.Text("<--更多命令", font = ("隶变-简", 17))]
                                            ]
